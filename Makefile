...
clean:
	@clear
	@rm -rf *.[og]* array* REPORT.html unit_test
test:
	@echo "==========================="
	...
	./unit_test
coverage:
	gcovr -r . --html-details -o array_coverage_report.html
rebuild: all
format:
	@clang-format -i *.cpp *.h -style=Google
cppcheck:
	cppcheck --enable=all --suppress=missingIncludeSystem *.cpp *.h array_cpps/* 
valgrind:
	valgrind --tool=memcheck --leak-check=yes ./array_test
